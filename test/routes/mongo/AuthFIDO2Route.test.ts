...
// test/auth/FIDO2Strategy.test.ts for isolated strategy-orchestration tests against a mocked
...
describe("Route:AuthFIDO2Mongo Tests", () => {
    const logger = Logger();
    const objectFactory: ObjectFactory = new ObjectFactory(config, logger);
    const server: Server = new Server({ config, basePath: "./test/server-mongo", logger, objectFactory });
    const baseUrl = "/mongo/auth/fido2";
    ...
    // `BaseAuthFIDO2Route.getCredentialById()` looks secrets up by their own `uid`.
    const createFIDO2SecretMongo = async function (data?: Partial<StoredPasskeyCredential> & { userUid?: string }) {
    ...
    it("Can begin a FIDO2 ceremony and receive challenge options.", async () => {
    ...
    it("Can complete a FIDO2 ceremony with a valid assertion and authenticate.", async () => {
        const user: UserMongo = await createUserMongo();
        const credentialId = uuid.v4();
        await createFIDO2SecretMongo({ id: credentialId, userUid: user.uid, counter: 5 });
