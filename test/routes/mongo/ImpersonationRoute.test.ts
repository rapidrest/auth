...
describe("Route:ImpersonationMongo Tests", () => {
    const logger = Logger();
    const objectFactory: ObjectFactory = new ObjectFactory(config, logger);
    const server: Server = new Server({ config, basePath: "./test/server-mongo", logger, objectFactory });
    const loginUrl = "/mongo/auth/password";
    const elevateUrl = "/mongo/auth/elevate";
    const impersonateUrl = "/mongo/admin/impersonate";
    const stopUrl = "/mongo/admin/impersonate/stop";
    ...
    const createSecretMongo = async function (userUid: string): Promise<SecretMongo> {
        const obj = new SecretMongo({ data: await hashPasswordForLogin("password", userUid), type: SecretType.PASSWORD, userUid });
        const result = await secretRepo.save(obj);
        await withOwnerACL(result.uid, "SecretMongo", userUid);
        return result;
    };
    ...
    // test/routes/mongo/AuthElevationRoute.test.ts uses. Overwrites the agent's `jwt` cookie with a
    // new, elevated one that carries trusted roles.
    ...
    it("Cannot impersonate without a trusted role, even when authenticated.", async () => {
        ...
        await createSecretMongo(user.uid);
        const target = await createUserMongo();
    ...
    it("Fails with a 400 when userUid is missing from the body.", async () => {
        const admin = await createUserMongo({ roles: ["admin"] });
        await createSecretMongo(admin.uid);
        const client = agent(server.getApplication());
