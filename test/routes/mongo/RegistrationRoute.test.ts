...
describe("Route:RegistrationMongo Tests", () => {
    ...
    it(
        ...
        async () => {
            ...
            const otherUser: UserMongo = await userRepo.save(new UserMongo({ roles: [], scopes: [], verified: false }));
            await aliasRepo.save(
                new AliasMongo({ alias: email, type: AliasType.EMAIL, userUid: otherUser.uid, verified: false }),
            );
