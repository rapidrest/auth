...
///////////////////////////////////////////////////////////////////////////////
// Isolated unit tests for BaseAuthPasskeyRoute — no HTTP server, no database.
import { RepoUtils } from "@rapidrest/service-core";
import { PasskeyStrategy } from "../../src/auth/PasskeyStrategy.js";
import { BaseAuthPasskeyRoute } from "../../src/routes/BaseAuthPasskeyRoute.js";
...
class TestAuthPasskeyRoute extends BaseAuthPasskeyRoute<any, any, any> {
...
function makeMockObjectFactory(aliasRepo: any, secretRepo: any, userRepo: any, userUtils: any) {
    const newInstance = vi.fn(async (type: any, opts: any) => {
        ...
        if (type === PasskeyStrategy) {
            return new PasskeyStrategy(opts.args[0]);
        }
        return undefined;
    });
    return { newInstance };
...
describe("BaseAuthPasskeyRoute Tests", () => {
    ...
    it("Registers a PasskeyStrategy under its name once initialized.", async () => {
        ...
        expect(register).toHaveBeenCalledWith("passkey", expect.any(PasskeyStrategy));
    });
    ...
    describe("getCredentials", () => {
        ...
        it("Returns the .data of all matching PASSKEY secrets for the user.", async () => {
            ...
            expect(find).toHaveBeenCalledWith(
                { type: SecretType.PASSKEY, userUid: "user-uid-1" },
                { ignoreACL: true },
            );
