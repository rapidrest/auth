...
describe("Route:AuthFIDO2SQL Tests", () => {
    const logger = Logger();
    const objectFactory: ObjectFactory = new ObjectFactory(config, logger);
    const server: Server = new Server({ config, basePath: "./test/server-sql", logger, objectFactory });
    const baseUrl = "/sql/auth/fido2";
    ...
    const createFIDO2SecretSQL = async function (data?: Partial<StoredPasskeyCredential> & { userUid?: string }) {
    ...
    it("Can complete a FIDO2 ceremony with a valid assertion and authenticate.", async () => {
        ...
        await createFIDO2SecretSQL({ id: credentialId, userUid: user.uid, counter: 5 });
