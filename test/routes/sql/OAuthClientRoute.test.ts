...
///////////////////////////////////////////////////////////////////////////////
// Real-database integration coverage for BaseOAuthClientRoute — the isolated unit tests in
// test/routes/BaseOAuthClientRoute.test.ts mock repoUtils entirely and never exercise a real
// create-then-persist round trip. That gap is exactly how a real bug shipped undetected: `Client`
// originally had its own separate `clientId` field (meant to serve as the OAuth `client_id`), which
// `validateCreate()` never actually generated — every client persisted with the same empty-string
// default against that field's unique index, so the *second* client ever created would have failed
// outright. Every isolated unit test only ever created one mocked client, so nothing ever exercised
// the real persist-then-uniqueness-check path. Fixed by removing the redundant field entirely and
// using the record's own `uid` (already unique, auto-generated, and exposed in every API response)
// as the OAuth `client_id` everywhere — see every `client.clientId` → `client.uid` change across
// this library's OAuth routes/utils for where that value is actually used.
...
describe("Route:ClientSQL Tests", () => {
    const logger = Logger();
    const objectFactory: ObjectFactory = new ObjectFactory(config, logger);
    const server: Server = new Server({ config, basePath: "./test/server-sql", logger, objectFactory });
    const baseUrl = "/sql/oauth/clients";
    let repo: Repository<ClientSQL>;
    ...
    function newClientPayload(overrides: Partial<ClientSQL> = {}): Partial<ClientSQL> {
        return {
            clientName: "Test App",
    ...
    beforeAll(async () => {
        ...
        if (isSqlDataSource(conn)) {
            repo = conn.getRepository(ClientSQL);
        } else {
    ...
    it("Sets ownerUid to the caller and returns a one-time plaintext secret for a confidential client (with a non-trusted token).", async () => {
        ...
        const stored: ClientSQL | null = await repo.findOne({ where: { uid: result.body.uid } });
    ...
    it("Lets the owner read, update, and delete their own client; denies another non-trusted user.", async () => {
        ...
        const count = await repo.count({ where: { uid: clientUid } });
        expect(count).toBe(0);
    });
    ...
    it("Regenerates a confidential client's secret, invalidating the old one, for the owner.", async () => {
        ...
        const before: ClientSQL | null = await repo.findOne({ where: { uid: clientUid } });
        ...
        const after: ClientSQL | null = await repo.findOne({ where: { uid: clientUid } });
